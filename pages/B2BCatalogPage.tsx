        ...
        <B2BLayout>
            <div className="space-y-6">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    ...
                    <a
                        href="#/bayi/sepet"
                        className="inline-flex items-center gap-2 bg-emerald-500 text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-emerald-600 transition-all shadow-lg shadow-emerald-500/20"
                    >
                    ...
                    <div className="flex flex-col lg:flex-row gap-4">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-3 bg-slate-700/50 border border-slate-600/50 rounded-xl text-white placeholder-slate-500 focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500/50 transition-all"
                            />
                        </div>
                        <select
                ...
                {/* Product Count */}
                <p className="text-slate-400 text-sm">{filteredProducts.length} ürün bulundu</p>
                ...
                {viewMode === 'grid' ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {filteredProducts.map(product => {
                            ...
                            return (
                                <div key={product.id} className="bg-slate-800/50 backdrop-blur-xl rounded-2xl border border-slate-700/50 overflow-hidden hover:border-emerald-500/30 transition-all group">
                                    {/* Image */}
                                    <div className="aspect-square bg-slate-700/30 relative overflow-hidden">
                                        ...
                                        />
                                        {discountRate > 0 && (
                                            <div className="absolute top-3 left-3 bg-emerald-500 text-white text-xs font-bold px-2 py-1 rounded-lg">
                                                %{discountRate} Bayi İskonto
                                            </div>
                                        )}
                                        {cartQty > 0 && (
                                            <div className="absolute top-3 right-3 bg-emerald-500 text-white w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold">
                                                {cartQty}
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    {/* Content */}
                                    <div className="p-4">
                                        <p className="text-slate-500 text-xs font-medium mb-1">{product.brand_name}</p>
                                        <h3 className="text-white font-semibold text-sm mb-3 line-clamp-2">{product.name}</h3>
                                        ...
                                        {cartQty > 0 ? (
                                            ...
                                            <button
                                                ...
                                                className="w-full bg-emerald-500/10 text-emerald-400 py-2.5 rounded-xl font-bold text-sm hover:bg-emerald-500 hover:text-white transition-all flex items-center justify-center gap-2"
                                            >
                    ...
                    <div className="space-y-2">
                        {filteredProducts.map(product => {
                            ...
                            return (
                                <div key={product.id} className="bg-slate-800/50 backdrop-blur-xl rounded-xl border border-slate-700/50 p-4 hover:border-emerald-500/30 transition-all flex items-center gap-4">
                                    <img
                                        src={product.images?.[0] || '/placeholder.jpg'}
                                        alt={product.name}
                                    ...
                                    <div className="text-right flex-shrink-0">
                                        <p className="text-emerald-400 font-bold">₺{dealerPrice.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</p>
                                        {discountRate > 0 && (
                                            <p className="text-slate-500 text-xs line-through">₺{originalPrice.toLocaleString('tr-TR', { minimumFractionDigits: 2 })}</p>
                                        )}
                                    </div>
                                    <div className="flex items-center gap-2 flex-shrink-0">
                                        ...
                                        {cartQty > 0 && (
                                            <span className="text-white font-bold text-sm w-8 text-center">{cartQty}</span>
                                        )}
                                        <button
                                            onClick={() => addToB2BCart(product)}
                ...
                {/* Cart Toast */}
                {showCartToast && (
                    <div className="fixed bottom-6 right-6 bg-emerald-500 text-white px-6 py-3 rounded-xl shadow-2xl shadow-emerald-500/30 flex items-center gap-2 animate-bounce z-50">
