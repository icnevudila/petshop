...
import {
  ...
  X, Save, ChevronDown, Settings, Star,
    ...
    <div className="min-h-screen bg-gray-50 pt-28 pb-12">
      ...
      <div className="container mx-auto px-4">
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <h3 className="text-lg font-bold text-secondary mb-4">Popüler Ürünler</h3>
                ...
                {products.slice(0, 6).map(product => (
                  <div key={product.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500">₺{product.discounted_price || product.price} • Stok: {product.stock}</p>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <tbody>
                  {filteredProducts.slice(0, 20).map(product => (
                    <tr key={product.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="py-4 px-4"><span className={`px-3 py-1 rounded-full text-xs font-bold ${product.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{product.is_active ? 'Aktif' : 'Pasif'}</span></td>
        ...
        {activeTab === 'homepage' && (
          <div className="space-y-6">
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <h3 className="text-lg font-bold text-secondary mb-4">Anasayfa Özellikleri (Hızlı Kargo vs.)</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {homeFeatures.map((feature, idx) => (
                  <div key={feature.id} className="border border-gray-100 rounded-xl p-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <h3 className="text-lg font-bold text-secondary mb-4">Hızlı Erişim Kategorileri</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {modalType === 'product' && (<>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><label className="block text-sm font-bold text-gray-700 mb-2">İndirimli Fiyat</label>
                      <input type="number" value={productForm.discounted_price} onChange={(e) => setProductForm({ ...productForm, discounted_price: e.target.value })} className="w-full px-4 py-3 border border-gray-200 rounded-xl" /></div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                  ...
                  <div><label className="block text-sm font-bold text-gray-700 mb-2">Görseller (URL)</label>
                    <div className="flex gap-2 mb-2"><input type="text" value={newImage} onChange={(e) => setNewImage(e.target.value)} placeholder="Görsel URL yapıştır" className="flex-1 px-4 py-2 border border-gray-200 rounded-xl" />
                      <button onClick={addImageHandler} className="px-4 py-2 bg-gray-100 rounded-xl font-bold text-sm">Ekle</button></div>
                    <div className="flex flex-wrap gap-2">{productForm.images.map((img, i) => (
                      <div key={i} className="relative group"><img src={img} className="w-16 h-16 object-cover rounded-lg" />
                        <button onClick={() => setProductForm({ ...productForm, images: productForm.images.filter((_, idx) => idx !== i) })} className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white rounded-full text-xs">×</button></div>
                    ))}</div></div>
